...
def _mk_db(tmp_path):
    ...
    with engine.begin() as conn:
        ...
        conn.execute(sa.text("""CREATE TABLE indicator_values (id INTEGER PRIMARY KEY, symbol VARCHAR(30), market_type VARCHAR(20) NOT NULL DEFAULT 'linear', interval VARCHAR(10), open_time DATETIME, indicator_name VARCHAR(64), value FLOAT, calc_version VARCHAR(24))"""))
...
def test_build_recommendation_grid_and_params(tmp_path):
    ...
    with Session(engine) as db:
        ...
        db.execute(sa.text("INSERT INTO indicator_values (symbol, market_type, interval, open_time, indicator_name, value, calc_version) VALUES ('BTCUSDT','linear','60',:ot,'adx',10,'v1')"),{"ot":datetime.now(timezone.utc)})
        db.commit()
        rec = build_recommendation(db, 'BTCUSDT', 'linear', ['60'])
